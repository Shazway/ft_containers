...
/*   Updated: 2023/01/31 17:12:36 by tmoragli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	template<typename T, typename Compare, typename Alloc>
	bool	operator==(SetTree<T, Compare, Alloc> const& t1, SetTree<T, Compare, Alloc> const& t2)
	{
		typedef typename SetTree<T, Compare, Alloc>::iterator iterator;
	...
	class SetTree
	{
		...
		public:
			typedef SetTreeIterator<value_type>				iterator;
			typedef SetTreeConstIterator<value_type>			const_iterator;
		...
		public:
			SetTree(compare_type const& comp = compare_type(), allocator_type const& alloc = allocator_type()): _root(NULL),
			...
			SetTree(InputIterator first, InputIterator last, compare_type const& comp = compare_type(), allocator_type const& alloc = allocator_type()): _root(NULL), _size(0),
			...
			SetTree(SetTree const& copy): _root(NULL), _size(0), _sentinelStart(NULL), _sentinelEnd(NULL),
			...
			SetTree &operator=(SetTree const& assign)
			{
				__clear();
				if (_sentinelStart)
					destroy_node(_sentinelStart);
